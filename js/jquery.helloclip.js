; (function($, window, undefined) {
	...
	Clip.prototype = {
		display: function(self) {
			var html = '<form action="' + this.opts.uploadUrl + '" enctype="multipart/form-data" method="' + this.opts.uploadType + '">' + '<span>点击上传图片</span>' + '<input type="file" name="helloClipPic" class="helloClipPic" />' + '</form><canvas id="' + this.canvasID + '"></canvas>' + '<input type="hidden" name="helloClip-path" class="helloClip-path" value="css/img/hello-clip.jpg"/>';
			$(this.ele).css({
		...
		move: function(self) {
			$(self.ele).find('img').on("mousedown",function(e) {
				e.preventDefault();
				var downLeft = e.clientX - self.ele.offset().left,
				downTop = e.clientY - self.ele.offset().top;
