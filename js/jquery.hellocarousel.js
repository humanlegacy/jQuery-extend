	...
	Carousel.prototype = {
		display: function() {
			if (this.opts.size === 'fullScreen') {
				...
				this.container.css({
					width: this.opts.size[0] + 'px',
					height: this.opts.size[1] + 'px'
				});
			}
			this.item.css({
				width: this.width
			}).wrapAll('<div class="carousel-wrap"></div>');
		...
		moveLeft: function(self) {
			this.item.eq(this.oldindex).stop(false, true).animate({
				'left': this.width
			},500);
