;(function($, window, undefined) {
	function Table(ele, opts) {
		...
		this.$container,this.$header,this.$body,this.fixedHeight;
	}
	Table.prototype = {
		display:function(){
		...
		colFixed:function(self){
			...
			this.$body.scroll(function(){
			   ...
			   self.$header.css({"margin-left":-left});
			   $(self.table.fixed).find(self.table.body).css({"margin-top":-top});
			});
		...
		rowFixed:function(self){
			...
			$(this.table.body).scroll(function(){
				var left = $(this).scrollLeft();
				$(self.table.fixed).css({"margin-left":-left});
			});
		...
		count:function(self){
			this.$header.find("td").each(function(index){
			   var Hw = $(this).find("span").width(),Bw = self.$body.find("td").eq(index).find("span").width();
			   if(Hw > Bw){
				   self.$body.find("td").eq(index).find("span").css({width:Hw+'px'});
			   }else{
				   $(this).find("span").css({width:Bw+'px'});
			   }
			});
			this.$body.css({
